...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  if(!username){
    return res.status(400).json(
      new ApiResponse(400, "username is missing")
    )
  }
  ...
  if(!channel?.length){
    return res.status(404).json(
      new ApiResponse(404, "Channel not exist!")
    )
  }
...
export { registerUser, loginUser, logoutUser, refreshAccessToken, chanegePassword, getCurrentUser, updateUserAvatar, updateAccountDetails, getUserChannelProfile, getWatchHistory }